    ...
    public partial class Form1 : Form
    {
        ...
        private void disperrinf(byte errcode)
        {
            switch (errcode)
            {
                ...
                case 28:
                    ...
                    break;
                default:
                    MessageBox.Show("返回码错误代码:" + errcode, "示例提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            switch (cardtype)
            {
                ...
                case 3:
                    ...
                    byte status = tagbuf_addtext(languagecodestr, languagecodestrlen, textstr, textstrlen); 
                    if (status == 0)
                    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            switch (cardtype)
            {
                ...
                case 3:
                    tagbuf_clear(); //清空标签数据缓冲
                    byte status = tagbuf_adddata(typestr, typestrlen, datastr, datastrlen); //可以用此方法写入多条记录
                    if (status == 0)
                    {
                        myctrlword = 0x80 + 0x10;
                        status = piccwrite_ndeftag(myctrlword, mypiccserial, oldpicckey, newpicckey);
                        ...
                        {
                            pcdbeep(38);
                            MessageBox.Show("NDEF数据标签写入成功！！", "示例提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else { disperrinf(status); }
                    }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            switch (cardtype)
            {
                ...
                case 3:
                    tagbuf_clear(); //清空标签数据缓冲
                    byte status = tagbuf_adduri(languagecodestr, languagecodestrlen, titlestr, titlestrlen, uriheaderindex, uristr, uristrlen);
                    if (status == 0)
                    {
                        myctrlword = 0x80 + 0x10;
                        status = piccwrite_ndeftag(myctrlword, mypiccserial, oldpicckey, newpicckey);
            ...
            switch (cardtype)
            {
                ...
                case 3:
                    ...
                    if (status == 0)
                    {
                        ...
                        {
                            pcdbeep(38);
                            MessageBox.Show("NDEF呼叫电话写入成功！！", "示例提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else { disperrinf(status); }
                    }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            switch (cardtype)
            {
                ...
                case 3:
                    tagbuf_clear(); //清空标签数据缓冲
                    byte status = tagbuf_addbusinesscard(infostr, infostrlen);  //可以用此方法写入多条记录
                    if (status == 0)
                    {
                        myctrlword = 0x80 + 0x10;
                        status = piccwrite_ndeftag(myctrlword, mypiccserial, oldpicckey, newpicckey);
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            switch (cardtype)
            {
                ...
                case 3:
                    tagbuf_clear(); //清空标签数据缓冲
                    byte status = tagbuf_addwifi(ssidstr, ssidstrlen, authtype, crypttype, keystr, keystrlen);  //可以用此方法写入多条记录
                    if (status == 0)
                    {
                        myctrlword = 0x80 + 0x10;
                        status = piccwrite_ndeftag(myctrlword, mypiccserial, oldpicckey, newpicckey);
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            switch (cardtype)
            {
                ...
                case 3:
                    tagbuf_clear(); //清空标签数据缓冲
                    byte status = tagbuf_addbluetooth(blenamestr, blenamestrlen, macbuf);   //可以用此方法写入多条记录
                    if (status == 0)
                    {
                        myctrlword = 0x80 + 0x10;
                        status = piccwrite_ndeftag(myctrlword, mypiccserial, oldpicckey, newpicckey);
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            switch (cardtype)
            {
                ...
                case 3:
                    tagbuf_clear(); //清空标签数据缓冲
                    byte status = tagbuf_addapp(packagestr, packagestrlen);  //可以用此方法写入多条记录
                    if (status == 0)
                    {
                        myctrlword = 0x80 + 0x10;
                        status = piccwrite_ndeftag(myctrlword, mypiccserial, oldpicckey, newpicckey);
                        ...
                        {
                            pcdbeep(38);
                            MessageBox.Show("NDEF启动应用写入成功！！", "示例提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else { disperrinf(status); }
                    }
