...
'//易写版_增加文件（文件序号0~5，文件只读密码：长度,文件读写密码,长度，分配空间)
Public Declare Function cpursfileadd_s Lib "OUR_MIFARE.dll" (ByVal fileno As Byte, ByVal readonlykey As Long, ByVal readonlykeylen As Byte, ByVal writekey As Long, ByVal writekeylen As Byte, ByVal customsize As Long) As Byte
'
'//易写版_写文件数据(文件序号,文件读写密码,密码长度,写入起始位置,写入长度，写入内容)
Public Declare Function cpursfiledatawrite_s Lib "OUR_MIFARE.dll" (ByVal fileno As Byte, ByVal keytype As Byte, ByVal key As Long, ByVal keylen As Byte, ByVal startaddr As Long, ByVal databuf As Long, ByVal datalen As Long) As Byte
